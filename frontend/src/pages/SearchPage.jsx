...
export default function SearchPage() {
  ...
  const sort = params.get('sort') || (collection === 'best-sellers' ? 'popular' : 'newest')
  const limit = 24
  ...
  const isSectionBrowse = Boolean(section)
  const sectionConfig = useMemo(() => {
    const sections = Array.isArray(content?.homepage?.productSections) ? content.homepage.productSections : []
    return sections.find(s => String(s?.id || '') === String(section)) || null
  }, [content, section])

  // Focus input on mount
  useEffect(() => {
    if (!isSectionBrowse) inputRef.current?.focus()
  }, [])

  // Keep term in sync with q param
  useEffect(() => {
    setTerm(query)
  }, [query])

  // Debounced update of q param when typing
  useEffect(() => {
    if (isSectionBrowse) return
    const t = setTimeout(() => {
      ...
      if (next !== (query || '')) {
        try {
          ...
          console.warn('Failed to update search params', e)
        }
      }
    }, 300)
    return () => clearTimeout(t)
  }, [term, query, params, setParams, isSectionBrowse])

  useEffect(() => {
    let ignore = false
    async function run() {
      if (isSectionBrowse) {
        const ids = Array.isArray(sectionConfig?.productIds) ? sectionConfig.productIds.map(String).filter(Boolean) : []
        if (ids.length === 0) {
          setData({ products: [], pagination: { page: 1, pages: 1, total: 0 } })
          return
        }
        setLoading(true)
        setError('')
        try {
          ...
          if (!ignore) setData({ products: Array.isArray(products) ? products : [], pagination: { page: 1, pages: 1, total: Array.isArray(products) ? products.length : 0 } })
        } catch (e) {
          console.warn('Failed to load section products:', e?.message)
          if (!ignore) setData({ products: [], pagination: { page: 1, pages: 1, total: 0 } })
        } finally {
      ...
      try {
        const res = hasSearch
          ? await apiService.getProducts({ search: query.trim(), page, limit, sort })
          : await apiService.getProducts({ page, limit, sort })
        const products = res?.data?.products || []
        const pagination = res?.data?.pagination || { page: 1, pages: 1, total: products.length }
        if (!ignore) setData({ products, pagination })
      } catch (e) {
        console.warn('Falling back to local products for search/browse:', e?.message)
    ...
    run()
    return () => { ignore = true }
  }, [query, collection, section, page, sort, limit, fallbackProducts, isSectionBrowse, sectionConfig])
  ...
  const onChangeSort = (e) => {
    try {
      ...
      console.warn('Failed to update sort param', err)
    }
  }
  ...
  const onChangePage = (nextPage) => {
    try {
      ...
      console.warn('Failed to update page param', err)
    }
  }

  const title = useMemo(() => {
    if (query) return `Search results for "${query}"`
    if (isSectionBrowse) return String(sectionConfig?.title || 'Products')
    if (collection === 'new-arrivals') return 'New Arrivals'
    if (collection === 'best-sellers') return 'Best Sellers'
    return 'Search'
  }, [query, collection, isSectionBrowse, sectionConfig])

  // Build suggestions from current results
  const suggestions = useMemo(() => {
    ...
    return {
      categories: Array.from(cats).slice(0, 6),
      subcategories: Array.from(subs).slice(0, 8)
    }
  }, [data])
  ...
  return (
    <main className="w-full px-4 sm:px-6 lg:px-8 py-6">
      ...
      {/* Suggestions */}
      {!isSectionBrowse && query && (suggestions.categories.length > 0 || suggestions.subcategories.length > 0) && (
        <div className="mt-4 space-y-3">
          ...
          {suggestions.subcategories.length > 0 && (
            <div>
              <div className="text-xs font-semibold text-gray-700 mb-2">Subcategories</div>
              <div className="flex flex-wrap gap-2">
                {suggestions.subcategories.map(s => (
                  <button
                    key={s}
                    onClick={() => navigate(`/category/${encodeURIComponent(suggestions.categories[0] || '')}?sub=${encodeURIComponent(s)}`)}
                    className="px-2 py-1 text-xs border rounded hover:bg-gray-50"
                  >{s}</button>
                ))}
              </div>
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {data.products.map(p => {
          ...
          if (img === '/placeholder.jpg') {
            ...
            if (firstImage?.url) foundImage = true
          }
