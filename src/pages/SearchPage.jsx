...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError('')
      try {
        const res = await apiService.getProducts({ search: query.trim(), page, limit, sort })
        const products = res?.data?.products || []
        const pagination = res?.data?.pagination || { page: 1, pages: 1, total: products.length }
        if (!ignore) setData({ products, pagination })
      } catch (e) {
        console.warn('Falling back to local products for search:', e?.message)
    ...
    run()
    return () => { ignore = true }
  }, [query, page, sort, limit, fallbackProducts])
  ...
  return (
    <main className="w-full px-4 sm:px-6 lg:px-8 py-6">
      ...
      {/* Suggestions */}
      {query && (suggestions.categories.length > 0 || suggestions.subcategories.length > 0) && (
            ...
            <div key={pid} className="group block">
              ...
              <div className="mt-2 flex items-center justify-between gap-2">
                <div className="flex-1 pl-2">
                  ...
                  <div className="text-xs text-gray-700">€ {Number(p.price || 0).toFixed(2)}</div>
                </div>
                <div>
                  <button
                    onClick={() => toggle(pid)}
